...
public sealed class McpTests
{
    ...
    public async Task HttpEndpointAuthenticatesRealMcpClientAndRevokesAccess()
    {
        await WithStore(async store =>
        {
            ...
            foreach (var name in new[] { "get_action_guide", "create_response_draft", "create_reply_draft", "create_forward_draft", "search_drive", "set_mail_state", "read_mail_headers", "read_mail_raw", "get_mail_link", "cancel_mail_action", "recover_mail_action", "search_discovered_people", "list_workspace_accounts", "list_calendars", "list_events", "create_event", "create_event_from_mail", "update_event", "delete_event", "search_contacts", "create_contact", "update_contact", "delete_contact", "list_task_lists", "list_tasks", "create_task", "update_task", "set_task_completed", "delete_task", "create_task_list", "rename_task_list", "delete_task_list", "list_notebooks", "list_note_sections", "list_note_pages", "read_note_page", "create_note_page", "update_note_page", "delete_note_page" })
                Assert.Contains(available, tool => tool.Name == name);
            var guide = await client.CallToolAsync("get_action_guide", new Dictionary<string, object?> { ["topic"] = "mail" }, cancellationToken: TestContext.Current.CancellationToken);
            Assert.False(guide.IsError == true);
            Assert.Contains("create_response_draft", Assert.IsType<TextContentBlock>(Assert.Single(guide.Content)).Text);
