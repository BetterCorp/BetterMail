...
public sealed partial class MainWindowViewModel : ViewModelBase
{
    ...
    private int _unifiedMessageCount;
        ...
        Func<TimeSpan, CancellationToken, Task>? waitForMarkRead = null)
    {
        ...
        ToggleReadCommand = new AsyncCommand(ToggleReadAsync, CanRunSelectedMailAction, allowConcurrent: true);
        ArchiveCommand = new AsyncCommand(() => MoveSelectedMessageAsync("archive", "Archiving...", "Archived"), CanRunSelectedMailAction, allowConcurrent: true);
        DeleteCommand = new AsyncCommand(() => MoveSelectedMessageAsync("deleteditems", "Moving to Deleted Items...", "Moved to Deleted Items"), CanRunSelectedMailAction, allowConcurrent: true);
        JunkCommand = new AsyncCommand(() => MoveSelectedMessageAsync("junkemail", "Moving to Junk Email...", "Moved to Junk Email"), CanRunSelectedMailAction, allowConcurrent: true);
        NotJunkCommand = new AsyncCommand(() => MoveSelectedMessageAsync("inbox", "Moving to Inbox...", "Marked as not junk"), CanRunSelectedMailAction, allowConcurrent: true);
        ...
        MoveMailboxUpCommand = new AsyncCommand<MailboxLayoutItem>(item => MoveMailboxAsync(item, -1), item => OrderedMailboxes.FirstOrDefault()?.Id != item.Mailbox.Id);
        MoveMailboxDownCommand = new AsyncCommand<MailboxLayoutItem>(item => MoveMailboxAsync(item, 1), item => OrderedMailboxes.LastOrDefault()?.Id != item.Mailbox.Id);
        ...
        SearchCommand = new AsyncCommand(() => StartGlobalSearchAsync(debounce: false, forceOpen: true), () => _store is not null, allowConcurrent: true);
    ...
    public ObservableCollection<MailAction> BusyActions { get; } = [];
    ...
    public bool IsDraftsView
    {
        ...
        private set
        {
            if (SetProperty(ref _isDraftsView, value))
            {
                ...
                RaisePropertyChanged(nameof(IsUnifiedSection));
                ...
                RaisePropertyChanged(nameof(ShowDraftRows));
    ...
    public bool IsUnifiedInbox => IsUnifiedSection && _unifiedFilter == MailMessageFilter.All;
    ...
    public bool ShowDraftRows => IsDraftsView && !IsOutboxView;
    public bool HasOutbox => BusyActions.Count > 0;
    public string OutboxCountText => BusyActions.Count > 0 ? BusyActions.Count.ToString("N0") : "";
    public string DraftEmptyTitle => IsOutboxView ? "Nothing pending" : "No drafts";
    ...
    public string CurrentItemCountText => IsDraftsView
        ? IsOutboxView ? $"{BusyActions.Count:N0} pending" : $"{VisibleDrafts.Count:N0} draft{(VisibleDrafts.Count == 1 ? "" : "s")}"
        : MessageCountText;
    public string DraftCountText => Drafts.Count == 0 ? "Drafts" : $"Drafts ({Drafts.Count:N0})";
    public bool HasDrafts => Drafts.Count > 0;
    public int SyncIssueCount => Drafts.Count(static draft => draft.HasSyncIssue);
    public int DraftConflictCount => Drafts.Count(static draft => draft.HasSyncConflict);
    public string SyncIssueCountText => SyncIssueCount > 0 ? SyncIssueCount.ToString("N0") : "";
    public string DraftConflictCountText => DraftConflictCount > 0 ? DraftConflictCount.ToString("N0") : "";
    public int PinnedMessageCount => _pinnedMessageCount;
    public int FlaggedMessageCount => _flaggedMessageCount;
    public string UnifiedMessageCountText => _unifiedMessageCount > 0 ? _unifiedMessageCount.ToString("N0") : "";
    public string PinnedMessageCountText => PinnedMessageCount > 0 ? PinnedMessageCount.ToString("N0") : "";
    public string FlaggedMessageCountText => FlaggedMessageCount > 0 ? FlaggedMessageCount.ToString("N0") : "";
    ...
    public string CurrentFolderName => IsSearchResultsView
        ? "Search results"
        : IsDraftsView
            ? _draftListFilter switch
            {
                DraftListFilter.SyncIssues => "Sync issues",
                DraftListFilter.Conflicts => "Draft conflicts",
                DraftListFilter.Outbox => "Busy",
                _ => "Drafts"
            }
            : _unifiedFilter switch
            {
                MailMessageFilter.Pinned => "Pinned",
                MailMessageFilter.Flagged => "Flagged",
                _ => _selectedFolder?.DisplayName ?? "Inbox"
            };
    public double MessageRowHeight => (IsCompact ? 58 : 78) + (IsSearchResultsView ? 18 : 0);
    ...
    public string TogglePinText => SelectedMessage?.IsPinned == true ? "Unpin" : "Pin";
    ...
    public string SearchResultSummary => _mailSearchSummary;
    ...
    public SettingsTabItem? SelectedSettingsTab
    {
        ...
        set
        {
            if (SetProperty(ref _selectedSettingsTab, value))
            {
                ...
                RaisePropertyChanged(nameof(ShowMcpSettings));
    ...
    public bool ShowMcpSettings => SelectedSettingsTab?.Name == "MCP";
    ...
    {
        IsSettingsOpen = true;
        await Mcp.RefreshMailboxesAsync();
    ...
    private async Task StartGlobalSearchAsync(bool debounce, bool forceOpen = false)
    {
        ...
        if (_store is null || query.Length == 0)
        {
        ...
        SearchQuery parsed;
        IReadOnlyList<MailFolderKey> searchFolders;
        try
        {
        ...
        try
        {
            ...
            if (SelectedSearchScope is "Everything" or "Mail")
            {
                var localMailSearch = AddGlobalResultsAsync(
                    "Mail",
                    SearchCachedMailGloballyAsync(parsed, searchFolders, source.Token),
                    source);
                searches.Add(localMailSearch);
                if (!parsed.HasMailFilters && query.Length > 0)
        ...
        CancellationTokenSource source)
    {
        ...
        foreach (var mailbox in Mailboxes.Where(MailboxMatchesSearchFilters))
        {
            ...
            try
            {
                ...
                {
                    var local = await _store.GetMessageAsync(message.MailboxId, message.ProviderId, source.Token);
                    if (local is not null && IsSearchableMail(local)) remoteMessages.Add(local);
                }
            }
            catch (Exception exception) when (exception is not OperationCanceledException)
            {
    ...
    private async Task<IReadOnlyList<GlobalSearchResult>> SearchPeopleGloballyAsync(
        ...
        CancellationToken cancellationToken)
    {
        var allowedMailboxes = Mailboxes.Where(MailboxMatchesSearchFilters).Select(mailbox => mailbox.Id).ToArray();
        var discoveredTask = _store!.GetDiscoveredPeopleAsync(query, 8, cancellationToken, allowedMailboxes);
    ...
    internal string MailLocation(MailMessage message)
    {
        var mailbox = Mailboxes.FirstOrDefault(candidate => candidate.Id == message.MailboxId);
    ...
    private async Task OpenGlobalSearchResultAsync(GlobalSearchResult result)
    {
        ...
        switch (result.Module)
        {
            case "Mail" when result.Value is MailMessage message:
                ...
                break;
            case "People":
                ModuleSearchText = SearchQuery.Parse(SearchText).Text;
            ...
            case "To Do":
                ...
                {
                    TasksWorkspace.SearchText = SearchQuery.Parse(SearchText).Text;
        ...
        IReadOnlyList<MailMessage>? results = null)
    {
        ...
        if (results is not null && selected is not null)
        {
            ...
            _displayedMailAccount = new SearchAccountFilter(SearchGroupFor(selected), null, selected.MailboxId);
        }
        RaisePropertyChanged(nameof(SearchResultSummary));
    ...
    internal int MessageListContextVersion => _messageLoadVersion;
    ...
    private async Task LoadMessagesAsync(bool showLoading = true)
    {
        ...
        try
        {
            ...
            if (IsSearchResultsView && _displayedMailQuery is { } displayedQuery)
            {
    ...
    private async Task LoadFoldersAsync()
    {
        ...
        if (structureChanged)
        {
            ...
            var expanded = FolderGroups.ToDictionary(group => group.Mailbox.Id, group => group.IsExpanded);
            Replace(FolderGroups, OrderedMailboxes.Select(mailbox => new MailboxFolderGroup(
                ...
                { IsExpanded = expanded.GetValueOrDefault(mailbox.Id, !_collapsedMailboxes.Contains(mailbox.Id)) }));
        }
        else
        {
    ...
    private async Task ShowUnifiedFilterAsync(MailMessageFilter filter)
    {
        IsSettingsOpen = false;
        ActiveModule = "Mail";
        IsDraftsView = false;
        IsSearchResultsView = false;
        _unifiedFilter = filter;
        SetSelectedFolder(null);
        Messages.Clear();
        SelectedMessage = null;
        RaisePropertyChanged(nameof(CurrentFolderName));
    ...
    private async Task RefreshUnifiedCountsAsync()
    {
        ...
        RaisePropertyChanged(nameof(UnifiedMessageCountText));
        RaisePropertyChanged(nameof(PinnedMessageCount));
        RaisePropertyChanged(nameof(FlaggedMessageCount));
        RaisePropertyChanged(nameof(PinnedMessageCountText));
    ...
    internal void ComposeTo(PersonEntry person)
    {
        ...
        {
            ComposeRequested?.Invoke(new ComposeRequest(To: new MailAddress(person.DisplayName, person.PrimaryEmail).ToString()));
        }
    }
    ...
    private Task ToggleReadAsync() => ToggleReadMessagesAsync(ActionMessages());

    private Task ToggleReadMessagesAsync(IReadOnlyList<MailMessage> messages) =>
        QueueMessageStateChangesAsync(messages, read: messages.Count > 0 ? !messages[0].IsRead : null);
        ...
        string successStatus)
    {
        ...
        try
        {
            ...
            {
                if (!TryGetMessageContext(message, out var account, out var mailbox)) continue;
                var destination = Folders.FirstOrDefault(folder => folder.MailboxId == mailbox.Id &&
                    (folder.ProviderId == destinationFolderId || folder.WellKnownName == destinationFolderId))?.Folder
            ...
            foreach (var message in messages)
            {
                ...
                if (displayed is not null && displayed.FolderId == message.FolderId)
                {
                    ...
                    foreach (var selected in SelectedMessages.Where(item => SameMessage(item, displayed)).ToArray()) SelectedMessages.Remove(selected);
                    if (wasCurrent && (SelectedMessage is null || SameMessage(SelectedMessage, displayed)))
                        SelectedMessage = Messages.Count == 0 ? null : Messages[Math.Min(index, Messages.Count - 1)];
                }
                ConversationThread.Reconcile(ConversationThread.Threads.SelectMany(thread => thread.Messages)
                    .Select(item => item.Message).Where(candidate => !SameMessage(candidate, message)));
            }
            RaiseMessageState();
            Status = successStatus;
            _ = SyncAsync();
        }
        catch (Exception exception) { Error = exception.Message; Status = "Could not queue move"; }
        finally { EndMessageFeedback(messages); }
    }

    private Task ToggleFlagAsync() => ToggleFlagMessagesAsync(ActionMessages());

    private Task ToggleFlagMessagesAsync(IReadOnlyList<MailMessage> messages) =>
        QueueMessageStateChangesAsync(messages, flagged: messages.Count > 0 ? !messages[0].IsFlagged : null);

    private Task TogglePinAsync() => TogglePinMessagesAsync(ActionMessages());
    private Task TogglePinMessagesAsync(IReadOnlyList<MailMessage> messages) =>
        QueueMessageStateChangesAsync(messages, pinned: messages.Count > 0 ? !messages[0].IsPinned : null);
    ...
    private async Task ViewHeadersAsync(MailMessage? message)
    {
    ...
    private void ApplyMessageStateUpdate(
        MailMessage message,
        bool? isRead = null,
        bool? isFlagged = null,
        bool? isPinned = null)
    ...
    {
        MessageListReconciling?.Invoke();
        IsReconcilingMessages = true;
        try
        {
        ...
        finally
        {
            ...
            MessageListReconciled?.Invoke();
        }
    }

    internal bool IsReconcilingMessages { get; private set; }
    internal event Action? MessageListReconciled;
    internal event Action? MessageListReconciling;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return message is null
            ? null
            : await LoadAttachmentContentAsync(message, attachment, cancellationToken);
    }

    internal async Task<IReadOnlyList<MailAttachment>> GetAttachmentsAsync(
        MailMessage message,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var attachments = await _provider.GetAttachmentsAsync(
                account, mailbox, message.ProviderId, cancellationToken);
            return message.Body?.Contains("cid:", StringComparison.OrdinalIgnoreCase) == true
                ? await HydrateAttachmentsAsync(
                    account,
                    mailbox,
                    message.ProviderId,
                    attachments,
    ...
    public async Task DeleteLocalDraftAsync(string id)
    {
        if (_store is null)
        {
            throw new InvalidOperationException("The encrypted mail store is unavailable. The draft has not been deleted.");
        }
        ...
        await _store.QueueDraftDeletionAsync(draft);
        if (!_draftSyncLocks.TryGetValue(draft.MailboxId, out var draftLock) || draftLock.CurrentCount != 0)
        {
            var current = await _store.GetLocalDraftAsync(id);
            if (current?.ProviderDraftId is null)
            {
                var deletion = (await _store.GetMailActionsAsync()).FirstOrDefault(action =>
                    action.Kind == MailActionKind.DeleteDraft && action.ItemId == id);
                if (deletion is not null) await _store.CompleteDraftDeletionAsync(deletion, null);
            }
        }
        await RefreshDraftsAsync();
        _ = SyncAsync();
    ...
    private async Task<LocalDraft?> GetStoredDraftAsync(string id) =>
        _store is null
            ? null
            : await _store.GetLocalDraftAsync(id);
    ...
    {
        if (_store is not null && await _store.IsDraftPendingDeletionAsync(draft.Id)) return;
...
public sealed class MailboxFolderGroup(Mailbox mailbox, IReadOnlyList<MailFolderNode> folders) : ViewModelBase
{
    private bool _isExpanded = true;
    public Mailbox Mailbox => mailbox;
    public IReadOnlyList<MailFolderNode> Folders => folders;
